"use strict";
// File generated from our OpenAPI spec by Stainless. See CONTRIBUTING.md for details.
Object.defineProperty(exports, "__esModule", { value: true });
exports.Actions = void 0;
const resource_1 = require("../../core/resource.js");
const path_1 = require("../../internal/utils/path.js");
/**
 * Endpoints related to porting orders management.
 */
class Actions extends resource_1.APIResource {
    /**
     * Activate each number in a porting order asynchronously. This operation is
     * limited to US FastPort orders only.
     *
     * @example
     * ```ts
     * const response =
     *   await client.portingOrders.actions.activate(
     *     '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
     *   );
     * ```
     */
    activate(id, options) {
        return this._client.post((0, path_1.path) `/porting_orders/${id}/actions/activate`, options);
    }
    /**
     * Requests cancellation of the porting order and returns the updated order.
     *
     * @example
     * ```ts
     * const response = await client.portingOrders.actions.cancel(
     *   '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
     * );
     * ```
     */
    cancel(id, options) {
        return this._client.post((0, path_1.path) `/porting_orders/${id}/actions/cancel`, options);
    }
    /**
     * Confirms the porting order and submits it for processing. Make sure all required
     * information and documents are attached before confirming.
     *
     * @example
     * ```ts
     * const response = await client.portingOrders.actions.confirm(
     *   '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
     * );
     * ```
     */
    confirm(id, options) {
        return this._client.post((0, path_1.path) `/porting_orders/${id}/actions/confirm`, options);
    }
    /**
     * Creates a sharing token for a porting order. The token can be used to share the
     * porting order with non-Telnyx users.
     *
     * @example
     * ```ts
     * const response = await client.portingOrders.actions.share(
     *   '182bd5e5-6e1a-4fe4-a799-aa6d9a6ab26e',
     * );
     * ```
     */
    share(id, body, options) {
        return this._client.post((0, path_1.path) `/porting_orders/${id}/actions/share`, { body, ...options });
    }
}
exports.Actions = Actions;
//# sourceMappingURL=actions.js.map